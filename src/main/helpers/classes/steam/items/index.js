...
class items {
  ...
  inventoryConverter(inventoryResult, isCasket = false) {
    ...
    for (const [key, value] of Object.entries(inventoryResult)) {
      ...
      const musicIndexBytes = getAttributeValueBytes(value, 166);
      if (musicIndexBytes) {
        value.music_index = musicIndexBytes.readUInt32LE(0);
      }
      const graffitiTint = getAttributeValueBytes(value, 233);
      if (graffitiTint) {
      ...
      const equipped = this.handleError(this.itemProcessorisEquipped, [value]);
  ...
  itemProcessorName(storageRow, imageURL) {
    ...
    if (storageRow.music_index !== undefined) {
      ...
      const nameToUse = `Music Kit | ${this.getTranslation(
        musicKitResult.loc_name
      )}`;
    ...
    if (defIndexresult.item_name !== undefined) {
      ...
      const baseSkinName = this.getPrefab(defIndexresult.prefab).item_name;
    ...
    if (
      ...
      if (relevantStickerData.slot == 0 && baseOne.includes('Coin') == false) {
    ...
    if (storageRow.attribute !== undefined) {
      for (const [, value] of Object.entries(storageRow.attribute)) {
        if (value.def_index == 140 && finalName.includes('Souvenir') == false) {
  ...
  itemProcessorCanBeMoved(returnDict, storageRow) {
    ...
    if (returnDict.item_url.includes('crate_key') && storageRow.flags == 10) {
      return false;
    }
  ...
  checkIfAttributeIsThere(item, attribDefIndex) {
    const attrib = (item.attribute || []).find(
