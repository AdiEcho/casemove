...
const moveFromReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case 'MOVE_TO_ADD_TO':
      ...
      const casketAlreadyExists =
        state.activeStorages.indexOf(action.payload.casketID) > -1;
    ...
    case 'MOVE_TO_TOTAL_TO_ADD':
      const toMoveAlreadyExists = state.totalToMove.filter(
        (row) => row[0] != action.payload.itemID
      );
